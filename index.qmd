...
mf_map(poi_reproj, type = "typo", var = "amenity", cex = .7, pch = 15, 
       ...
       add = TRUE)
mf_map(pt_reproj, pch = 24, cex = 1.3, lwd = 2, col = "darkblue",
       add = TRUE)
```
...
Le package `spatstat` [@R-spatstat] regroupe de nombreuses fonctions pour analyser des semis de points. Il utilise (génère ?) des objets de classe **ppp** *(planar point pattern)*, dans des fenêtres d'observation, de classe **owin** *(observation window)*.
...
mf_map(r_poly, var = "lyr.1", breaks = quantile(r_poly$lyr.1),
       border = NA, lwd = .2, type = "choro", alpha = 0.6, pal = "Mint",
       leg_title = "Densité\n (n/ha)", add = TRUE)
mf_map(poi_reproj, pch = 20, cex = .2, col = "black", add = TRUE)
mf_map(res$zone, col = NA, border = "#c6bab1", lwd = 4, add = TRUE)
```
...
for(i in 1:nrow(d)){
  matcar[[i]] <- osrmTable(src = o[, c("X", "Y")],
                        dst = d[i , c("X", "Y")],
                        measure = "duration",
                        osrm.profile = 'foot')
  Sys.sleep(5)
  matcar[[i]]$ID_ORIG <- c(1:nrow(o))
  matcar[[i]]$ID_DEST <- rep(i, nrow(o))
  matcar[[i]] <- data.frame("ID_ORIG" = matcar[[i]]$ID_ORIG,
                         "ID_DEST" = matcar[[i]]$ID_DEST,
                         "duration" = matcar[[i]]$durations)
  colnames(matcar[[i]]) <- c("ID_ORIG", "ID_DEST", "dur_car")
}


for(i in 1:nrow(d)){
  matbike[[i]] <- osrmTable(src = o[, c("X", "Y")],
                        dst = d[i , c("X", "Y")],
                        measure = "duration",
                        osrm.profile = "bike")
  Sys.sleep(5)
  matbike[[i]]$ID_ORIG <- c(1:nrow(o))
  matbike[[i]]$ID_DEST <- rep(i, nrow(o))
  matbike[[i]] <- data.frame("ID_ORIG" = matbike[[i]]$ID_ORIG,
                         "ID_DEST" = matbike[[i]]$ID_DEST,
                         "duration" = matbike[[i]]$durations)
  colnames(matbike[[i]]) <- c("ID_ORIG", "ID_DEST", "dur_car")
}

matcar <- do.call(rbind, matcar)
matbike <- do.call(rbind, matbike)
write.csv(matcar, "data/matcar.csv")
write.csv(matbike, "data/matbike.csv")
```
