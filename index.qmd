...
mf_annotation(pt_reproj, txt = loc)
...
# as.numeric(object.size(osm_pt) / 1000000) # 491 Mb
...
Le code pour effectuer les calculs est présenté ci-dessous, mais a été joués en local. Les résultats sont enregistrés dans le dossier data (fichier *matfoot.csv*, geopackage *mat.gpkg*).
...
C'est un autre engin de routage
...
mf_map(grid, var = "n_resto", type = "choro", breaks = c(0,1,5,10,30,40),
...
st_write(routes, "data/mat.gpkg", layer = "routes")
```
...
#| fig-width: 7
#| fig-height: 7
routes <- st_read("data/mat.gpkg", layer = "routes")
...
st_write(isos, "data/mat.gpkg", layer = "isos")
```
...
trip <- trip[[1]]$trip
st_write(trip, "data/mat.gpkg", layer = "trip")
```
...
#| fig-width: 7
#| fig-height: 7
trip <- st_read("data/mat.gpkg", layer = "trip")
